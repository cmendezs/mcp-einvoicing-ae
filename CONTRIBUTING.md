# Contributing to mcp-einvoicing-ae
...
```bash
uv run ruff check src/mcp_einvoicing_ae/ tests/ audit/
uv run ruff format --check src/mcp_einvoicing_ae/ tests/ audit/
uv run mypy src
```
...
```bash
uv run ruff check --fix src/mcp_einvoicing_ae/ tests/ audit/
uv run ruff format src/mcp_einvoicing_ae/ tests/ audit/
```
...
- [ ] Validation fixes reference the relevant rule ID (e.g. `[NEED: PINT AE rule id, from the supplied specification]`)
...
Please open an issue at https://github.com/cmendezs/mcp-einvoicing-ae/issues and include:
